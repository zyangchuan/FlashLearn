  ...
  mutations: {
    ...
    insertCard(state, payload) {
      if (state.studyCards.length == 1) {
        ...
        state.studyCards.splice(
          state.studyCards.length - payload.noOfCardsAfter,
          0,
          payload.card
        );
      }
  ...
  actions: {
    ...
    async loadCards(context, deckid) {
      const loadCards_config = {
        headers: {
          Authorization:
            context.state.currentUser.signInUserSession.idToken.jwtToken,
        },
        params: {
          deckid: deckid,
        },
      ...
      return axios
        .get(process.env.VUE_APP_API + "/card-decks/study", loadCards_config)
        .then((response) => {
