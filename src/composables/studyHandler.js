...
const studyHandler = () => {
  ...
  const notFamiliar = async () => {
    ...
    try {
      ...
      store.commit("insertCard", {card: currentCard.value, noOfCardsAfter: 1})
      currentCard.value = store.getters.getCurrentCard
      return axios.put('https://f4ng7av2s6.execute-api.ap-southeast-1.amazonaws.com/flashlearn-test/card-decks/study', cardInfo, updateCard_config)
    } catch (error) {
      throw error
    }
  }
  ...
  const notSure = async () => {
    ...
    const cardInfo = {
      ...
      try {
        store.commit("popCard")
        store.commit("insertCard", {card: currentCard.value, noOfCardsAfter: 2})
        currentCard.value = store.getters.getCurrentCard
        return axios.put('https://f4ng7av2s6.execute-api.ap-southeast-1.amazonaws.com/flashlearn-test/card-decks/study', cardInfo, updateCard_config)
      } catch(error) {
        throw error
